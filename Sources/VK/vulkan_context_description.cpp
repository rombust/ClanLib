...
namespace clan
{
	...
	bool VulkanContextDescription::get_debug() const
	{
	...
	const std::vector<std::string> &VulkanContextDescription::get_device_extensions() const
	{
		return impl->device_extensions;
	}
	bool VulkanContextDescription::get_sampler_anisotropy() const
	{
	...
	{
		impl->instance_extensions.push_back(name);
	}
	void VulkanContextDescription::add_device_extension(const std::string &name)
	{
		impl->device_extensions.push_back(name);
	}
