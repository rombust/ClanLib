...
namespace clan
{
	VulkanElementArrayBufferProvider::VulkanElementArrayBufferProvider() {}
	VulkanElementArrayBufferProvider::~VulkanElementArrayBufferProvider() {}

	void VulkanElementArrayBufferProvider::create(int size, BufferUsage usage)
	{
		create(nullptr, size, usage);
	}

	void VulkanElementArrayBufferProvider::create(void *data, int size, BufferUsage usage)
	{
		if (!vk_device)
			throw Exception("VulkanElementArrayBufferProvider: set_device() must be called before create()");

		bool is_static = (usage == BufferUsage::static_draw);
	...
	void VulkanElementArrayBufferProvider::copy_from(GraphicContext &gc,
	...
	void VulkanElementArrayBufferProvider::copy_to(GraphicContext &gc,
													TransferBuffer &transfer_buffer,
													int dest_pos, int src_pos, int size)
	{
