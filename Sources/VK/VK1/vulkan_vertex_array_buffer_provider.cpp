...
namespace clan
{
	VulkanVertexArrayBufferProvider::VulkanVertexArrayBufferProvider() {}
	VulkanVertexArrayBufferProvider::~VulkanVertexArrayBufferProvider() {}

	void VulkanVertexArrayBufferProvider::create(int size, BufferUsage usage)
	{
		create(nullptr, size, usage);
	}

	void VulkanVertexArrayBufferProvider::create(void *data, int size, BufferUsage usage)
	{
		if (!vk_device)
			throw Exception("VulkanVertexArrayBufferProvider: set_device() must be called before create()");
	...
	void VulkanVertexArrayBufferProvider::copy_to(GraphicContext &gc,
												TransferBuffer &transfer_buffer,
												int dest_pos, int src_pos, int size)
	{
